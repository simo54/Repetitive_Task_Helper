...
def git_status():
    logging.info("START git_process")
    print("Beginning of git_process")

    # Changing directory goes here
    # os.chdir(path.path_name)

    try:
        git_process = subprocess.Popen(cmd.git_cmd, stdout=subprocess.PIPE)
        while git_process.poll() is None:
            output = git_process.stdout.readline()
            output_decode = output.strip()
            if git_process.poll() is not None and output == '':
                break
            if output:
                print(output_decode.decode())
        result_code = git_process.poll()
        if result_code != 0:
            raise Exception
    except Exception:
        print(errors.generic_error)
        logging.error("ERROR, process has been stopped")
        logging.info("END git_process")
        raise
    finally:
        logging.info("END git_process")
        print("git_process complete \n")
